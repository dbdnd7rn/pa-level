...
type FirestoreListingInput = {
  title?: string;
  propertyType?: string;
  monthlyFrom?: number | string | null;
...
function mapFirestoreListing(id: string, data: FirestoreListingInput): Listing {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          setListing(mapFirestoreListing(snap.id, snap.data() as FirestoreListingInput));
      ...
      try {
        ...
        const others: Listing[] = snap.docs
          ...
          .map((docSnap) =>
            mapFirestoreListing(
              docSnap.id,
              docSnap.data() as FirestoreListingInput
            )
          )
          .slice(0, 3);
